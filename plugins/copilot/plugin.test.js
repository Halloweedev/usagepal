...
function setKeychainToken(ctx, token) {
  ctx.host.keychain.readGenericPassword.mockImplementation((service) => {
    if (service === "UsagePal-copilot") return JSON.stringify({ token });
    return null;
  });
}
...
describe("copilot plugin", () => {
  ...
  it("loads token from UsagePal keychain", async () => {
    const ctx = makePluginTestContext();
  ...
  it("does not persist token loaded from UsagePal keychain", async () => {
    const ctx = makePluginTestContext();
  ...
  it("falls back when UsagePal keychain payload lacks token field", async () => {
    const ctx = makePluginTestContext();
    ctx.host.keychain.readGenericPassword.mockImplementation((service) => {
      if (service === "UsagePal-copilot") return JSON.stringify({ notToken: "x" });
