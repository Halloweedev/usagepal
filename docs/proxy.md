...
UsagePal can route provider and plugin HTTP requests through an optional proxy.

- Supported proxy types: `socks5://`, `http://`, `https://`
- Config file: `~/.usagepal/config.json`
- Default: off
- UI: none

## Config File

Create `~/.usagepal/config.json`:
...
- Restart UsagePal after changing the file.
...
This applies to provider and plugin HTTP requests that go through UsagePal's built-in HTTP client.
