...
Use this when UsagePal is not working and you need to share debug info.
...
1. In your macOS menu bar, find the UsagePal icon.
2. Right-click it (or hold `Control` and click).
3. Open `Debug Level`.
4. Select `Debug`.

If UsagePal does not open at all, skip this step and continue.
...
```text
~/Library/Logs/com.halloweed.usagepal
```
...
1. Attach `usagepal.log`.
2. If you also see files like `usagepal.log.1`, attach those too.
