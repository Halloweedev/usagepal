 ...
 * preview in onboarding. Remount with `key={milestone}` to replay the slide-in. */
export function MacNotificationBanner({ milestone }: { milestone: PaceMilestone }) {
  const meta = MILESTONE_META[milestone]
  return (
    <div
      data-testid="notification-banner"
      className="mx-auto flex w-full max-w-sm items-center gap-3 rounded-2xl border bg-card px-4 py-3 shadow-lg animate-in slide-in-from-top-4 fade-in duration-300"
    >
